...
const topLimitVerticalFall = topLimitRightFall.concat(topLimitLeftFall.slice(13)).sort((a,b) => a-b)
//
...
const bottomLimitVerticalFall = bottomLimitRightFall.concat(bottomLimitLeftFall.slice(0,11)).sort((a,b) => a-b)
...
function limitSwitch() {
    topLimitTriangles =
    verticalFallEnabled ? topLimitVerticalFall : 
    currentFall === leftFall ? topLimitLeftFall : topLimitRightFall

    bottomLimitTriangles =
    verticalFallEnabled ? bottomLimitVerticalFall :
    currentFall === leftFall ? bottomLimitLeftFall : bottomLimitRightFall
}
